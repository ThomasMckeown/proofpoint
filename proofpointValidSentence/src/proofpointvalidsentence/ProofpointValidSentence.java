...
public class ProofpointValidSentence {
    ...
    //combining search for quotations and periods so time isn't wasted doing multiple searches through the sentence 
    public String amountOfQutationsAndPeriods(String sentence) {

        int count = 0;
        int count2 = 0;

        //first checks if the sentence ends with a period. if it doesnt then immediately returns invalid
        //so time isn't wasted going through the sentence looking for quotation marks and periods
        if (sentence.trim().endsWith(".")) {
            ...
            //using modulus to check if the remainder when count is divided by 2 is 0
    ...
    public String numbersBelow13AreText(String sentence) {
        ...
        //goes through the while loop if it finds numbers
        while (matcher.find()) {

            //parsing the number found in the string to an int so it can be checked
        ...
        //this check is done in case there is no numbers in the sentence so no match would be found
